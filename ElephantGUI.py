...
class dbconnect(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.connectbutton = tk.Button(self.master, text='Connect', command=self.connect_to_db).grid(row=5, column=1, sticky=tk.NSEW, padx=5, pady=5)
        self.disconnectbutton = tk.Button(self.master, text='Disconnect', command=self.disconnect_from_db).grid(row=5, column=2, sticky=tk.NSEW, padx=5, pady=5)
...
class read_elephant_file(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.analysebutton = tk.Button(self.master, text='Analyse', command=self.reload_file).grid(row=2, column=2, sticky=tk.NSEW, padx=5, pady=5)
    ...
    def call_read_elephants(self):
        ...
        file_content = read_elephants(self.name, ',')
        n_accepted = file_content[1].__len__()
        n_rejected = file_content[3].__len__()
        parse_reads(file_content, prefix=(self.name.partition('.')[0]))
        self.result.config(state=tk.NORMAL)
        self.result.delete(1.0,tk.END)
